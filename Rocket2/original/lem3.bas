...
9 REM THERE IS ABSOLUTELY NO CHANCE INVOLV#End Region
10 Z$="GO"
15 B1=1
20 M=17.95
25 F1=5.25
...
240 IF K=1 THEN 250
245 GOTO 220
250 Z=1852.8 REM meters in a nautical mile
255 M$="METERS"
260 G3=3.6
265 N$=" KILOMETERS"
270 G5=1000
275 GOTO 305
280 Z=6080 REM feet in a nautical mile
285 M$="FEET"
290 G3=0.592
295 N$="N.MILES"
300 G5=Z
...
325 PRINT "THE LUNAR EXCUSION MODULE, YOU WILL BE EXPECTED TO"
...
375 PRINT "0 (ZERO) DEGREES IS DIRECTLY DOWARDS THE MOON."
380 PRINT
...
415 PRINT
420 PRINT TAB(27);"SURFACE OF THE MOON"
425 PRINT
430 PRINT
...
545 PRINT "        DISTANCE FROM LANDING SITE IN ";N$
550 PRINT "        VERTICAL VELOCITY IN ";M$;"/SECOND"
555 PRINT "        HORIZONTAL VELOCITY IN ";N$;"/SECOND"
...
605 IF ABS(F-0.05)>1 THEN 945
610 IF ABS(F-0.05)<0.05 THEN 945
...
630 N=T1/20
635 T1=T1/N
640 P=P*3.14159/180 REM angle in radiians
645 S=SIN(P)
650 C=COS(P)
655 M2=M0*T1*F/B
660 R3=-0.5*R0*((V0/R)**2)+R*A1*A1
665 A3=-2*R1*A1/R
670 FOR I=1 TO N
...
725   M=M-0.5*M2
730   R4=R3
735   R3=-0.5*R0*((V0/R)**2)+R*A1*A1
740   R2=(3*R3-R4)/2+0.00526*F1*F*C/M
745   A4=A3
750   A3=-2*R1*A1/R
755   A2=(3*A3-A4)/2+0.0056*F1*F*S/(M*R)
760   X=R1*T1+0.5*R2*T1*T1
765   R=R+X
770   H0=H0+X
775   R1=R1+R2*T1
780   A=A+A1*T1+0.5*A2*T1*T1
785   A1=A1+A2*T1
790   M=M-0.5*M2
795   T=T+T1
800   IF H0<3.287828E-04 THEN 810
805 NEXT I
810 H=H0*Z REM Altitude in Z units
815 H1=R1*Z REM Vertical velocity in Z units per second
820 D=R0*A*Z REM Distance from landing site
825 D1=R*A1*Z REM Horizontal velocity in Z units per second
830 T2=M1*B/M0 REM Fuel units remaining
834 REM ~~~ report ~~~
835 PRINT " ";T;TAB(10);H;TAB(23);D;
840 PRINT TAB(37);H1;TAB(49);D1;TAB(60);T2
844 REM check for landing
845 IF H0<3.287828E-04 THEN 880; REM landing
849 REM check for escape trajectory
850 IF R0*A>164.4736 THEN 1050; REM Adios
854 REM M1 is the fuel level. If Fuel remains then repeat the burn
...
1075 PRINT
1080 PRINT "BUT MISSED THE LANDING SITE BY";ABS(D/G5);M$
1085 GOTO 1100
1090 PRINT
