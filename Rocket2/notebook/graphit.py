...
def dograph(csvfile,window,outfile) :
    ...
    avg_landing_speed = surface_hits['vertspeed'].mean()
    std_landing_speed = surface_hits['vertspeed'].std()
    ...
    ax1r = ax1.twinx()
    reward_x = surface_hits.index.to_numpy()
    reward_y = surface_hits['avgreward'].to_numpy()
    ...
    #
    ax0 = plt.subplot2grid(plotgrid, (0,0), sharex=ax1, rowspan=1, colspan=4)
    y1 = -surface_hits['vertspeed'].values
    ...
    y1mean = -surface_hits['vertspeed'].rolling(window).mean().values
    ax0r = ax0.twinx()
    c2 = ax0r.plot(reward_x,y1mean,top_line_color,label='Running Avg', linewidth=1.25)
    ax0r.tick_params(axis='y',color=top_line_color)

    #
    # Second Top graph is the Horizontal speed
    #
    ax3 = plt.subplot2grid(plotgrid, (1,0), sharex=ax1, rowspan=1, colspan=4)
    y1 = surface_hits['horizspeed'].values
    ax3.plot(reward_x, y1, top_graph_color, linewidth=1.1)
    ax3.axhline(1, color=h_line_color)
    ax3.set_yticks([0])
    ...
    y1mean = surface_hits['horizspeed'].rolling(window).mean().values
    ax3r = ax3.twinx()
    c2 = ax3r.plot(reward_x,y1mean,top_line_color,label='Running Avg', linewidth=1.25)
    ax3r.tick_params(axis='y',color=top_line_color)
    ...
    y2mean = surface_hits['fuel'].rolling(window).mean().values
    ax2r = ax2.twinx()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--slack", help="true if slack output is used",type=str2bool, default=False)
